...
class CSWinTransv2_threeIn(nn.Module):
    ...
    def __init__(self, class_num, droprate, decouple, infonce, block=4):
        ...
        for i in range(self.block):
            ...
            setattr(self, clas, ClassBlock(1536, class_num, droprate=droprate))
    ...
    def forward(self, x):
        ...
        x = self.model.patch_embed(x) # 64, 64, 128
        ...
        x, ff = self.outpart(a, b, c, d) # logit vector,   embedding
    ...
    def cat(self, y1, y2, y3):
        ...
        for i in range(self.block):
            if i == 0:
                ...
                cat4 = torch.cat((y1[:, :, i], y2[:, :, i], y3[:, :, i],), dim=1)
    ...
    def outpart(self, a, b, c1, d):
        ...
        c = getattr(self, name)
        logit1, emb_a = c(a)
        logit_out.append(logit1)
        emb_out.append(emb_a)

        name = 'classifier1'
        c = getattr(self, name)
        logit2, emb_b = c(b)
        logit_out.append(logit2)
        emb_out.append(emb_b)

        name = 'classifier2'
        c = getattr(self, name)
        logit3, emb_c = c(c1)
        logit_out.append(logit3)
        emb_out.append(emb_c)

        name = 'classifier3'
        c = getattr(self, name)
        logit4, emb_d = c(d)
        logit_out.append(logit4)
        emb_out.append(emb_d)

        # embedding 
        ff = torch.stack(emb_out, dim=2)
        fnorm = torch.norm(ff, p=2, dim=1, keepdim=True) * np.sqrt(self.block)
        ff = ff.div(fnorm.expand_as(ff))
        ff = ff.view(ff.size(0), -1)

        if not self.training:
            return torch.stack(logit_out, dim=2), ff
        else:
            return logit_out, ff
