...
from hardsample_dataset import HardDataset
from pairdataset import PairDataset

from losses.triplet_loss import Tripletloss,TripletLoss, cosine_dist
...
parser.add_argument('--epoch', type=int, help='resume epoch')
...
def val(model, epoch, ):
    ...
    gallery_name_path = os.path.join('./model', opt.name, 'gallery_name.txt')
    query_name_path = os.path.join('./model', opt.name, 'query_name.txt')
    
    gallery_path = test_imgDatasets[gallery_name].imgs
    if not os.path.exists(gallery_name_path):
        f = open(gallery_name_path, 'w')
        for p in gallery_path:
            f.write(p[0] + '\n')
    
    query_path = test_imgDatasets[query_name].imgs
    if not os.path.exists(query_name_path):
        f = open(query_name_path, 'w')
        for p in query_path:
            f.write(p[0] + '\n')
    ...
    with torch.no_grad():
        mid_result_path = os.path.join('./model', opt.name, f'mid_result_{epoch}.mat')

        if not os.path.exists(mid_result_path):
...
def train_model_finetune(model, model_test, criterion, optimizer, scheduler, epoch, warm_up, warm_iteration, num_epochs=25):
    ...
    ranking_loss = nn.MarginRankingLoss(margin=0.3)

    epoch_loss = 0.0
    epoch_loss_triple = 0.0
    epoch_loss_ranking = 0.0
    epoch_loss_cls = 0.0
    total_sample = 0
    running_corrects = 0

    pbar = tqdm(total=len(dataloaders['drone']))
    for data_normal in dataloaders['drone']:
        img_d, img_s, label = [Variable(inp.cuda().detach()) for inp in data_normal]
        
        optimizer.zero_grad()

        now_batchsize = img_d.shape[0]

        # first inference
        logit_d, _, emb_d = model(img_d)
        logit_s, _, emb_s = model(img_s)

        preds, loss_logit1 = one_LPN_output(logit_d, label, criterion, opt.block)
        preds, loss_logit2 = one_LPN_output(logit_s, label, criterion, opt.block)
        loss_logit = loss_logit1 + loss_logit2 / 2

        feats = [emb_d, emb_s]
        emb_d, emb_s = [f.squeeze() for f in feats]
        split_num = opt.batchsize
        loss_triple = cal_triplet_loss(emb_d, emb_s, label, triplet_loss, split_num)
        
        loss = loss_logit + 10 * loss_triple
        ...
        preds, loss_logit1 = one_LPN_output(logit_d, label, criterion, opt.block)
        _, loss_logit2 = one_LPN_output(logit_s, label, criterion, opt.block)
        loss_logit = (loss_logit1 + loss_logit2) / 2
        ...
        cur_loss = loss.item() * now_batchsize
        cur_tripleloss = loss_triple.item() * now_batchsize
        cur_clsloss = loss_logit.item() * now_batchsize

        epoch_loss += cur_loss
        epoch_loss_triple += cur_tripleloss
        epoch_loss_cls += cur_clsloss
        total_sample += now_batchsize
        running_corrects += float(torch.sum(preds == label.data))
        
        pbar.update(1)
        pbar.set_postfix(loss='{:.6f}'.format(cur_loss), loss_triple='{:.6f}'.format(cur_tripleloss), loss_cls='{:.6f}'.format(cur_clsloss))


    epoch_loss /= total_sample
    epoch_loss_triple /= total_sample
    epoch_loss_cls /= total_sample
    acc = running_corrects / total_sample
    
    print(f'Epoch: {epoch} loss: {epoch_loss} loss_triple: {epoch_loss_triple} loss_ranking: {epoch_loss_ranking} ACC: {acc}')
...
for epoch in range(num_epochs - start_epoch):
    ...
    # train
    model, warm_up = train_model_finetune(model, model_test, criterion, optimizer_ft, exp_lr_scheduler, epoch, warm_up,
                                 warm_iteration,
